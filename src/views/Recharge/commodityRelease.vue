<template>
	<section>
		...
		<el-table :data="table" border highlight-current-row v-loading="listLoading" style="width: 100%;min-width: 1080px;">
			...
			<el-table-column prop="name" label="充值类型">
			</el-table-column>
			<el-table-column prop="goodsName" label="充值名称">
			</el-table-column>
			<el-table-column prop="price" label="价格">
			</el-table-column>
			<el-table-column prop="saleStatus"  :formatter="formatterstatus " label="状态">
			</el-table-column>
			...
			<el-table-column label="操作">
				<template scope="scope">
					...
					<!-- <el-button type="text" size="small" @click="statusBtn(scope.$index, scope.row)" v-if="scope.row.saleStatus === 2">启用</el-button> -->
					<!-- <el-button type="text" size="small" @click="statusBtnNo(scope.$index, scope.row)" v-if="scope.row.saleStatus === 1">禁用</el-button> -->
				...
				$.ajax({
	                ...
	                success:function(data){
	                  	...
	                  	for (var i = 0; i < info.length; i++) {
	                  		for (var x = 0; x < info[i].goodsList.length; x++) {
                  				const obj = {}
